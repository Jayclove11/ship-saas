...
export async function getAllFilesFrontMatter(folder: 'posts') {
  const prefixPaths = path.join(process.cwd(), 'src/blog-data', folder)
...
export function getFiles(type: 'posts') {
  const prefixPaths = path.join(process.cwd(), 'src/blog-data', type)
  const files = getAllFilesRecursively(prefixPaths)
...
export async function getFileBySlug<T>(type: 'posts', slug: string | string[]) {
  const mdxPath = path.join(process.cwd(), 'src/blog-data', type, `${slug}.mdx`)
  const mdPath = path.join(process.cwd(), 'src/blog-data', type, `${slug}.md`)
